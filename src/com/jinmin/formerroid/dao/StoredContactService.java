...
public class StoredContactService {
   ...
   public int checkDuplicatedStoredContact(String name, String tel){
      ...
      SQLiteDatabase db = sqlHelper.getReadableDatabase();
      try {
         Cursor cursor = db.rawQuery("select count(*) cnt  from " + SQLHelper.STORED_CONTACT_TB + " where name = '" + name + "' and tel = '" + tel + "'" ,null);
         while (cursor.moveToNext()) {
   ...
   public boolean insertContact(StoredContact storedContact) {
      ...
      long result = 0l;
      try {
         if( storedContact != null ) result = db.insert(SQLHelper.STORED_CONTACT_TB, null, storedContact.toContentValues());
      }
      catch (Exception e) {
